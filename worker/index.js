...
// api/user-marker.js
async function hashIP2(ip, salt) {
  if (!salt) {
    throw new Error("Salt is required for hashing");
  }
  const encoder = new TextEncoder();
...
async function onRequestGet2(context) {
  ...
  try {
    const clientIP = request.headers.get("CF-Connecting-IP") || request.headers.get("X-Forwarded-For") || request.headers.get("X-Real-IP") || "unknown";
    const salt = env.IP_HASH_SALT;
    const ipHash = await hashIP2(clientIP, salt);
    const existing = await env.DB.prepare(
      "SELECT id FROM markers WHERE ip_hash = ?"
    ).bind(ipHash).first();
...
// ../.wrangler/tmp/pages-gzjJX4/functionsRoutes-0.5731736646051324.mjs
var routes = [
  {
